...
void ply::findEdges() {
	...
	if (faceList != NULL) {
		...
		for (int i = 0; i < faceCount; i++) {
			...
			int vert2 = 1;
			for (int j = 0; j < faceList[i].vertexCount; j++) {
				edge* temp = new edge(NULL, i, -1, faceList[i].vertexList[vert1], faceList[i].vertexList[vert2]);


				if (!containsEdge(temp)) {
					cur->next = temp;
					cur = cur->next;
					edgeCount++;
				}
				else {
					int index = indexOfEdge(temp);
					edge* list = (*edgeList)->next;
...
bool ply::containsEdge(edge* nextEdge) {
	if (edgeList != NULL) {
		edge* cur = (*edgeList)->next;
		while (cur != NULL) {
			if (cur->faces[1] == -1 && cur->operator==(nextEdge)) {
				return true;
			}
			cur = cur->next;
		}
...
void ply::renderSilhouette(){
	...
	edge* cur = (*edgeList)->next;
	while(cur != NULL) {
		if (cur->faces[0] != -1 && cur->faces[1] != -1) {

			float diff = (faceList[cur->faces[0]].dotProd - faceList[cur->faces[1]].dotProd);
			if (diff > .5 || diff < -.5) {
				vertex vert = vertexList[cur->vertexes[0]];
				glVertex3f(vert.x, vert.y, vert.z);

				vert = vertexList[cur->vertexes[1]];
				glVertex3f(vert.x, vert.y, vert.z);
			}
		}
		cur = cur->next;
