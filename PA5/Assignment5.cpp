 ...
 * Date: 11/18/2018
 *
 * Description: Theres some threading here and there
*/
...
int  main_window;
string filenamePath = "data\\tests\\mirror_test.xml";
...
/*
  returns the index into sceneObjects of the intersection point
*/
int findClosestObj(Point isectPoint, Vector ray) {
	double minDist = MIN_ISECT_DISTANCE;
	int closestObject = -1;

	int workLoad = sceneObjects.size() / 4;
	int remainder = sceneObjects.size() % 4;

	for (int k = 0; k < sceneObjects.size(); k++) {
		Shape* shape = sceneObjects[k].shape;
		double curDist = shape->Intersect(isectPoint, ray, sceneObjects[k].transform);
...
/*
  determines if any objects intersect along the ray
*/
bool inShadow(Point isectPoint, Vector ray) {
	if (findClosestObj(isectPoint, ray) < 0) {
		return false;
	}
	else {
		return true;
...
/*
  calculate the color for an intersect point
*/
Point calculateColor(SceneObject closestObject, Vector normalVector, Vector ray, Point isectWorldPoint, int depth) {
	Point color;
	depth++;
	bool shadowed;

	int numLights = parser->getNumLights();
	//loop over all light scources
	for (int i = 0; i < numLights; i++) {
		...
		//do different things depending on the light type
		switch (lightData.type) {
		case LIGHT_POINT:
			lightDir = lightData.pos - isectWorldPoint;
			break;
		case LIGHT_DIRECTIONAL:
			lightPoint = Point(0,0,0) - (lightData.dir * 100000);
			lightDir = lightPoint - isectWorldPoint;
			break;
		case LIGHT_SPOT:
			lightDir = lightData.pos - isectWorldPoint;
			break;
		case LIGHT_AREA:
			...
			break;
		default:
			cerr << "unrecognized light type" << endl;
			exit(1);
		}
		lightDir.normalize();

		//if in shadow
		if (!(shadowed = inShadow(isectWorldPoint, lightDir))) {
			...
			//add in color of intersection
			for (int j = 0; j < 3; j++) {
				color[j] = color[j] + lightColor[j] * (diffuse[j] * dot_nl + specular[j] * power);
				if (color[j] > 1) {
					color[j] = 1.0;
				}
			}
	...
	//recursion end case
	if (depth < recursionDepth && !shadowed) {
		Vector recurRay = ray - (2 * dot(ray, normalVector) * normalVector);
		recurRay.normalize();

		int recurClosestObj;
		if ((recurClosestObj = findClosestObj(isectWorldPoint, recurRay)) != -1) {
			Matrix invTrans = sceneObjects[recurClosestObj].invTransform;
			double recurDist = sceneObjects[recurClosestObj].shape->Intersect(isectWorldPoint, recurRay, sceneObjects[recurClosestObj].transform);
			Vector normal = sceneObjects[recurClosestObj].shape->findIsectNormal(invTrans * isectWorldPoint, invTrans * recurRay, recurDist);
			normal = transpose(invTrans) * normal;
			normal.normalize();
			Point isectRef = isectWorldPoint + (recurDist - EPSILON) * recurRay;
			Point refColor = calculateColor(sceneObjects[recurClosestObj], normal, recurRay, isectRef, depth); //go to the next object

			Point reflector(closestObject.material.cReflective.r, closestObject.material.cReflective.b, closestObject.material.cReflective.g);
			for (int i = 0; i < 3; i++) {
				color[i] += refColor[i] * reflector[i]; //add in the reflections color
...
/*
 * runs every row of pixels in the screen and will calculate the color for that pixel
 */
void calcPixelHeight(int i) {
	for (int j = 0; j < pixelHeight; j++) {
		// cout << "computing: " << i << ", " << j << endl;

		Vector ray = generateRay(i, j);
		int closestObject = findClosestObj(camera->GetEyePoint(), ray);
		if (closestObject != -1) {
			if (isectOnly == 1) {
				setpixel(pixels, i, j, 255, 255, 255);
			}
			else {
				Matrix invTrans = sceneObjects[closestObject].invTransform;
				double minDist = sceneObjects[closestObject].shape->Intersect(camera->GetEyePoint(), ray, sceneObjects[closestObject].transform);
				Vector normal = sceneObjects[closestObject].shape->findIsectNormal(invTrans*camera->GetEyePoint(), invTrans*ray, minDist);
				normal = transpose(invTrans) * normal;
				normal.normalize();
				Point isectWorldPoint = camera->GetEyePoint() + (minDist - EPSILON)*ray;
				Point color = calculateColor(sceneObjects[closestObject], normal, ray, isectWorldPoint, 0);
				color = color * 255;
...
void callback_start(int id) {
	...
	for (int i = 0; i < pixelWidth; i++) {
		...
		//spawn a thread per row of pixels
		threads.push_back(thread(calcPixelHeight, i));
	}
...
int main(int argc, char* argv[])
{
	...
	GLUI_Spinner* recursion = glui->add_spinner("Depth", GLUI_SPINNER_INT, &recursionDepth);
	recursion->set_float_limits(0, 10);
