...
void ply::scaleAndCenter() {
	...
	for (int i = 0; i < vertexCount; i++) {
		if (max < abs(vertexList[i].x)) {
			max = abs(vertexList[i].x);
		}
		if (max < abs(vertexList[i].z)) {
			max = abs(vertexList[i].z);
		}
		if (max < abs(vertexList[i].y)) {
			max = abs(vertexList[i].y);
		}
		avgX += vertexList[i].x;
		avgY += vertexList[i].y;
		avgZ += vertexList[i].z;
      ...
      */
	  ...
	  for (int i = 0; i < faceCount; i++) {
		  ...
		  for (int j = 0; j < faceList[i].vertexCount; j++) {
			  glVertex3f(vertexList[faceList[i].vertexList[j]].x, vertexList[faceList[i].vertexList[j]].y, vertexList[faceList[i].vertexList[j]].z);
		  }
	  }
	  glEnd();
}
