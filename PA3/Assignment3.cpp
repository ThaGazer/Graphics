...
void renderObjects(SceneNode* root) {
	glPushMatrix();
	Matrix compositeMatrix;
	Matrix translate, rotate, scale, matrix;

	if (root->transformations.size() > 0) {
		for (int i = 0; i < root->transformations.size(); i++) {
			switch (root->transformations[i]->type) {
			case TRANSFORMATION_TRANSLATE:
				translate = trans_mat(root->transformations[i]->translate);
				break;
			case TRANSFORMATION_ROTATE:
				rotate = rot_mat(root->transformations[i]->rotate, root->transformations[i]->angle);
				break;
			case TRANSFORMATION_SCALE:
				scale = scale_mat(root->transformations[i]->scale);
				break;
			case TRANSFORMATION_MATRIX:
				matrix = root->transformations[i]->matrix;
				break;
			}
		}
		//compositeMatrix = rotate * scale * translate * matrix;
		compositeMatrix = translate * rotate * scale * matrix;
		glMultMatrixd(compositeMatrix.unpack());
	}

	if (root->primitives.size() > 0) {
		if (fillObj == 1) {
			applyMaterial(root->primitives[0]->material);
		}
		renderShape(root->primitives[0]->type);
	}

	for (int i = 0; i < root->children.size(); i++) {
		renderObjects(root->children[i]);
	}
	glPopMatrix();
}
